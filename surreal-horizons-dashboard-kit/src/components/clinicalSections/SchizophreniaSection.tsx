...
const SchizophreniaSection: React.FC<BaseSectionProps> = ({ user, rangeDays, language, chartColors }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const snap = await getDocs(collection(db, CLINICAL_COLLECTIONS.users, user.uid, CLINICAL_COLLECTIONS.schizophrenia));
        const list: Entry[] = [];
        snap.forEach(d => {
        ...
        <Card className="p-6 h-72">
          ...
          <ResponsiveContainer width="100%" height="85%">
            <BarChart data={negativeData} layout="vertical" margin={{ left: 20 }}>
              ...
              <Tooltip contentStyle={buildTooltipStyle(chartColors)} formatter={(v) => [`${Number(v).toFixed(1)}/5`, '']} />
