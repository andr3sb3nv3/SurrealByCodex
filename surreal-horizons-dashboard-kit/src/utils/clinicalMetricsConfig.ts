...
export const ENABLED_CLINICAL_METRICS = new Set<ClinicalMetricKey>(DEFAULT_CLINICAL_METRICS);
...
export const sanitizeClinicalMetrics = (raw: unknown): ClinicalMetricKey[] => {
  ...
  for (const item of raw) {
    ...
    if (!ENABLED_CLINICAL_METRICS.has(item as ClinicalMetricKey)) continue;
...
export const deriveClinicalMetricsFromIdentifiers = (identifiers: Array<string | null | undefined>): ClinicalMetricKey[] => {
  ...
  for (let i = 0; i < METRIC_ORDER_BY_DIGIT.length; i++) {
    ...
    const metric = METRIC_ORDER_BY_DIGIT[i];
    if (Number.isFinite(digit) && digit > 0 && ENABLED_CLINICAL_METRICS.has(metric)) {
      inferred.push(metric);
    }
  }
  return inferred;
};
